...
export default async function ViewProposalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!proposal) {
    return (
      <div style={{ textAlign: "center", padding: "3rem" }}>
        <h2 style={{ fontSize: "1.5rem", color: "#f43f5e" }}>Không tìm thấy dự trù</h2>
        <Link href="/sale/proposals" className="btn btn-secondary" style={{ marginTop: "1rem" }}>
          Quay lại danh sách
        </Link>
      </div>
    );
  ...
  const depreciationLabel = isThcs ? "4 năm THCS (2029-2030)" : "5 năm (2030-2031)";
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div className="screen-only" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "1.5rem" }}>
        <div>
          <Link href="/sale/proposals" style={{ color: "#64748b", textDecoration: "none", marginBottom: "0.5rem", display: "inline-flex", alignItems: "center", gap: "0.35rem", fontSize: "0.8rem", fontWeight: 500, transition: "color 0.2s" }}>
              ...
              {groupedItems.length === 0 ? (
                ...
                groupedItems.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td style={{ textAlign: "right", fontWeight: 700, color: "#ffffff" }}>{Number(item.totalPrice).toLocaleString()}</td>
                  </tr>
                ))
              )}
            </tbody>
