...
export default function EditConstructionForm({ construction }: { construction: Construction }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      unit: construction.unit || "Gói",
      standardPrice: String(construction.standardPrice || 0),
    },
  });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    setError("");
    try {
      const res = await updateOtherInvestment(construction.id, {
        ...data,
        category: "CONSTRUCTION"
      });
      if (res.success) {
        toast.success("Cập nhật Hạng mục Thi công thành công!");
    ...
    <div style={{ maxWidth: "680px", margin: "0 auto" }}>
      ...
      <div className="card" style={{ padding: "1.75rem" }}>
        <div style={{ paddingBottom: "1rem", marginBottom: "1.5rem", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <div>
            <h1 style={{ fontSize: "1.25rem", fontWeight: 800, color: "#ffffff" }}>Chỉnh sửa Hạng mục Thi công</h1>
            <p style={{ fontSize: "0.8rem", color: "#94a3b8" }}>Cập nhật tên gói thi công, mô tả chi tiết và đơn giá chuẩn</p>
          </div>
        </div>
