...
export default function EditSchoolForm({ school, sales }: { school: any; sales: any[] }) {
  ...
  const onSubmit = async (data: SchoolFormValues) => {
    ...
    try {
      const res = await updateSchool(school.id, data);
      if (res.success) {
    ...
    <>
      ...
      <div className="card" style={{ padding: "1.75rem" }}>
        <div style={{ paddingBottom: "1rem", marginBottom: "1.5rem", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <div>
            <h1 style={{ fontSize: "1.25rem", fontWeight: 800, color: "#ffffff" }}>Chỉnh sửa Thông tin Trường học</h1>
            <p style={{ fontSize: "0.8rem", color: "#94a3b8" }}>Cập nhật tên trường, địa chỉ và phân công Nhân viên Sale</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div style={{ marginTop: "2rem", display: "flex", gap: "1rem" }}>
            ...
            >
              {isLoading ? "Đang xử lý..." : "Cập nhật Trường học"}
            </button>
          </div>
        </form>
      </div>
